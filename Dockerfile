...
RUN curl -L -o /tmp/QmsgNtClient-NapCatQQ.zip https://cdn.262e.cn/QmsgNtClient-NapCatQQ/download/v$(curl https://cdn.262e.cn/QmsgNtClient-NapCatQQ/package.json | grep '"version":' | sed -E 's/.*([0-9]{1,}\.[0-9]{1,}\.[0-9]{1,}).*/\1/')/QmsgNtClient-NapCatQQ_$(arch | sed s/aarch64/arm64/ | sed s/x86_64/amd64/).zip
